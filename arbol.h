...
template<class T>
inline BinarySearchTree<T>::BinarySearchTree(T data, Aeropuerto* pa) {
    head = new Node<T>(data, pa);
}
...
template<class T>
Node<T>* BinarySearchTree<T>::insert(Node<T> *iterator, T data, Aeropuerto* pa) {
    if (isEmpty(iterator)) {
        iterator = new Node<T>(data, pa);
    } else if(data <= iterator->getData()){
        iterator->setLeft(this->insert(iterator->getLeft(), data, pa));
    } else {
        iterator->setRight(this->insert(iterator->getRight(), data, pa));
    }
    return iterator;
}

template<class T>
void BinarySearchTree<T>::insert(T data, Aeropuerto* pa) {
    head = this->insert(this->begin(), data, pa);
}
